...
export default function Admin() {
  ...
  async function loadResults(roundId) {
    ...
    if (fixtureIds.length === 0) {
      setMessage("This round currently has no fixtures.");
      setResultGames([]);
      setLoadingResults(false);
      return;
    }
  ...
  async function saveResult(game) {
    ...
    setMessage(
      `${game.home_team} ${result.home}-${result.away} ${game.away_team} saved.`
    );
  ...
  async function scoreRound() {
    ...
    if (resultGames.length === 0) {
      setMessage("This round has no fixtures to score.");
      return;
    }
    ...
    if (incomplete) {
      setMessage(
        `Enter and save all ${resultGames.length} results before scoring the round.`
      );
      return;
    }
    ...
    setScoring(true);
    setMessage(
      `Scoring round with ${resultGames.length} game${
        resultGames.length === 1 ? "" : "s"
      }...`
    );
    ...
    setMessage(
      `Round scored successfully using ${resultGames.length} game${
        resultGames.length === 1 ? "" : "s"
      }. Match points and competition points have been calculated.`
    );
    ...
    return (
      <main className="wrap">
        <div className="card">
          ...
          <p>
            Sign in with your Pick 7 administrator account.
          </p>
                ...
                {resultGames.length > 0 && (
                  <div className="notice">
                    {resultGames.length === 7
                      ? "This round has the normal 7 games."
                      : `This round has ${resultGames.length} games because one or more selected fixtures were postponed. No replacement game will be added.`}
                  </div>
                )}
